...
export default async function DeerHuntingPage() {
  ...
  return (
    <>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {features.map((feature) => (
              <div key={feature.title} className="flex items-start space-x-4">
                <div className="flex-shrink-0 w-12 h-12 bg-clay-100 rounded-lg flex items-center justify-center">
                  <svg className="w-6 h-6 text-clay-700" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                  </svg>
                </div>
                <div>
                  <h3 className="text-xl font-bold text-bark-800 mb-2">{feature.title}</h3>
                  <p className="text-bark-500">{feature.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Regulations */}
      <section className="section-padding bg-pine-50">
        <div className="container-custom">
          <SectionHeader
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden max-w-3xl mx-auto">
            {regulations.map((reg, index) => (
              <div
                key={reg.title}
                className={`p-6 ${index !== regulations.length - 1 ? 'border-b border-bark-200' : ''}`}
              >
                <h3 className="font-bold text-bark-800 mb-2">{reg.title}</h3>
                <p className="text-bark-500">{reg.content}</p>
              </div>
            ))}
          </div>
          ...
          <div className="text-center mt-8">
            <a
              ...
              className="text-pine-700 font-semibold hover:text-pine-800 inline-flex items-center"
            >
      ...
      <section className="py-16 bg-clay-700 text-white">
        <div className="container-custom text-center">
          <h2 className="text-3xl font-bold mb-4 text-white">Ready to Book Your Hunt?</h2>
